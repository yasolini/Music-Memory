<script lang="ts">
	...
	let shownIdA: null | string = $state(null);
	let shownIdB: null | string = $state(null);
	...
	function handleGame(card: Card) {
		if (shownIdA === null) {
			shownIdA = card.id;
			console.log(shownIdA);
		} else if (shownIdB === null) {
			shownIdB = card.id;
			console.log(shownIdB);
		} else if (
			shownIdA.charAt(0) + '.a' === shownIdB.charAt(0) + '.b' ||
			shownIdA.charAt(0) + '.b' === shownIdB.charAt(0) + '.a'
		) {
			console.log(shownIdA.charAt(0) + '.b');
			console.log('yes');
		} else {
			setTimeout(() => {
				shownIdA = null;
			...
			{#each cards as card}
				...
				>
					{#if shownIdA === card.id}
						<p>{card.value}</p>
					{/if}
					{#if shownIdB === card.id && shownIdA !== shownIdB}
						<p>{card.value}</p>
					{/if}
				</div>
			{/each}
